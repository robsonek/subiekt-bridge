...
| `GET /api/v1/bank-transactions?direction&unbooked_only&from&to` | Surowe przelewy z wyciągu (read-only); matching robi konsument |
| `GET /api/v1/open-receivables?direction&contractor_id&amount` | Otwarte rozrachunki (read-only) — druga strona do matchingu |
